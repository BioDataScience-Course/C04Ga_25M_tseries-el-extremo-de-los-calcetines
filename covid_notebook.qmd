...
# La série temporelle est créée (début le 15 février 2020 = 46e jour de l'année 2020)
covid_we_ts <- ts(covid_we$parks, start = c(2020, 46), frequency = 365)
plot(covid_we_ts)
# Aggrégation par moyenne des valeurs par semaine
covid_we_ts2 <- aggregate(covid_we_ts, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot(covid_we_ts2)
```
...
```{r hugotest}
covid_tsh <- ts(covid$workplaces, start = c(2020, 46), frequency = 365)
plot(covid_tsh, xlab = "Temps [année]", ylab = "Taux déplacement au travail p/r référence", main = "influence du COVID-19 sur le déplacement au travail")
```
