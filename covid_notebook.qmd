...
covid_sem_ts <- ts(covid_sem$parks, start = c(2020, 46), frequency = 365)
plot(covid_sem_ts)
# Aggrégation par moyenne des valeurs par semaine
covid_sem_ts2 <- aggregate(covid_sem_ts, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot(covid_sem_ts2)
```
...
plot(core_ts)
core_tss <- ts(covid_sem$commerce_recreation, start = c(2020, 46), frequency = 365)
core_ts2 <- aggregate(core_tss, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot(core_ts2)
```
...
covid_al_ts <- ts(covid$residential, start = c(2020, 46), frequency = 365)
plot(covid_al_ts)
...
```{r hugotest}
tsd(covid$workplaces_percent_change_from_baseline, type = "l")
```
